...
export class SwirlDB {
   ...
   */
  onDenied(callback: (reason: string) => void): void {
   ...
   * db.syncChanges();
   */
  insertListItem(path: string, index: number, value: any): void {
   ...
   * db.syncChanges();
   */
  spliceList(path: string, index: number, deleteCount: number, values: any[]): void {
